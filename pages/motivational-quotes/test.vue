<template>
    <div class="stories-container">
      ...
      <transition-group name="fade" tag="div" class="stories-grid">
        <div 
          ...
          class="story-card"
        >
          <div class="story-content" @click="toggleStory(story.id)">
          ...
          <button class="delete-button" @click.stop="deleteStory(story._id)">Delete</button>
        </div>
      </transition-group>
    ...
    methods: {
      ...
      async deleteStory(id) {
        try {
          ...
          if (result.success) {
            // Remove the story from both arrays
            this.stories = this.stories.filter(story => story.id !== id);
            this.filteredStories = this.filteredStories.filter(story => story.id !== id);
  
            // If the deleted story was currently selected, close the modal
            if (this.selectedStory && this.selectedStory.id === id) {
              this.selectedStory = null;
            }
          } else {
            console.warn(result.error || "Failed to delete the story.");
          }
        } catch (error) {
  ...
  .story-card {
    ...
    user-select: none;
    position: relative;
    padding-bottom: 50px; /* space for the delete button */
  }
